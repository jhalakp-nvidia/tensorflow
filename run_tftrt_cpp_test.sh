...
if [[ ! -z "${CPP_TEST_OP}" ]]; then
    FLAKY_FLAG="--flaky_test_attempts=1"
    BAZEL_TEST_PATH="//tensorflow/compiler/tf2tensorrt:convert_nodes_test"
else
    FLAKY_FLAG=""
    BAZEL_TEST_PATH="//tensorflow/compiler/tf2tensorrt/..."
fi

echo ""
echo "FLAKY_FLAG=${FLAKY_FLAG}"
echo "BAZEL_TEST_PATH=${BAZEL_TEST_PATH}"
sleep 2

# C++ Unittests
bazel test --verbose_failures $(cat ../nvbuildopts) ${bazel_cache_flag} \
  ${FLAKY_FLAG} --cache_test_results=no --local_test_jobs=$GPUS \
  --run_under=//tensorflow/tools/ci_build/gpu_build:parallel_gpu_execute \
  --test_filter="*${CPP_TEST_OP}*" \
  ${BAZEL_TEST_PATH}
